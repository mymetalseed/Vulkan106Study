...
public:
	...
	VulkanExample() :VulkanExampleBase(ENABLE_VALIDATION)
	{
		...
		//Values not set here are initialized in the base class constructo
	}
	...
	void buildCommandBuffers() {
		...
		for (int32_t i = 0; i < drawCmdBuffers.size(); ++i) {
			...
			//Ending the render pass will add an implicit barrier trasitioning the frame buffer color attachment to
	...
	void draw() {
		//Get the next image in the swap chain(back/front buffer)
	...
	// SRS - on other platforms use original bare code with local semaphores/fences for illustrative purposes
		...
		//Pass the semaphore singnaled by the command buffer submission from the submit info as the wait semaphore for swap chain presentation
	...
	void prepareVertices(bool useStagingBuffers) {
		//A note on memory managment in Vulkan in general
		...
		if (useStagingBuffers) {
			...
			//Create destination before with device only visiblity
